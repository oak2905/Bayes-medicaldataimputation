...
##
from bayesian.models import BayesianLinear, BayesianGRUCell, BayesianLSTMCell
...
class MLPContext(nn.Module):
    def __init__(self, num_vars, context_hidden):
        ...
        self.context_mlp = nn.Sequential(
            ...
            BayesianLinear(2*context_hidden, context_hidden, prior_sigma_1 = prior_sigma_10, 
            prior_sigma_2=prior_sigma_20, prior_pi =prior_pi0, 
            bias=bias0, posterior_rho_init = posterior_rho_init0,
            posterior_mu_init = posterior_mu_init0, weight_mu_init = 0.1,bias_mu_init=0.1)
        )
...
class MLPFeatureImputation(nn.Module):
    def __init__(self, input_size, hidden_size=32):
        ...
        self.nonlinear_regression = nn.Sequential(
            ...
            BayesianLinear(hidden_size, 1, prior_sigma_1 = prior_sigma_10, 
                prior_sigma_2=prior_sigma_20, prior_pi =prior_pi0, bias=bias0, posterior_rho_init = posterior_rho_init0,
                posterior_mu_init = posterior_mu_init0, weight_mu_init = 0.1,bias_mu_init=0.1)
        )
...
class RNNContext(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.rnn_cell = BayesianGRUCell(input_size, hidden_size, prior_sigma_1 = prior_sigma_10, 
            prior_sigma_2=prior_sigma_20, prior_pi =prior_pi0, 
            bias=bias0, posterior_rho_init = posterior_rho_init0,
            posterior_mu_init = posterior_mu_init0, weight_mu_init = 0.1,bias_mu_init=0.1)
...
class BayesCATSI(nn.Module):
    def __init__(self, num_vars, hidden_size=64, context_hidden=32):
        ...
        self.initial_hidden = BayesianLinear(2*context_hidden, 2*hidden_size, prior_sigma_1 = prior_sigma_10, 
            prior_sigma_2=prior_sigma_20, prior_pi =prior_pi0, 
            bias=bias0, posterior_rho_init = posterior_rho_init0,
            posterior_mu_init = posterior_mu_init0, weight_mu_init = 0.1,bias_mu_init=0.1)
        ...
        self.rnn_cell_forward = BayesianLSTMCell(2*num_vars+2*context_hidden, hidden_size, prior_sigma_1 = prior_sigma_10, 
            prior_sigma_2=prior_sigma_20, prior_pi =prior_pi0, 
            bias=bias0, posterior_rho_init = posterior_rho_init0,
            posterior_mu_init = posterior_mu_init0, weight_mu_init = 0.1,bias_mu_init=0.1, peephole = peephole)
        self.rnn_cell_backward = BayesianLSTMCell(2*num_vars+2*context_hidden, hidden_size, prior_sigma_1 = prior_sigma_10, 
            prior_sigma_2=prior_sigma_20, prior_pi =prior_pi0, 
            bias=bias0, posterior_rho_init = posterior_rho_init0,
            posterior_mu_init = posterior_mu_init0, weight_mu_init = 0.1,bias_mu_init=0.1, peephole = peephole)
    ...
    def forward(self, data):
        ...
        rnn_loss = self.recurrent_impute.sample_elbo(
        ...
        feat_loss = self.feature_impute.sample_elbo(
        ...
        fusion_loss = self.fuse_imputations.sample_elbo(
                inputs=torch.cat((gamma, masks), dim=-1),
                labels=normalized_values * masks,
                criterion=criterion_loss,
                sample_nbr=sample_nbr,  # Number of samples from the posterior distribution
                complexity_cost_weight=1 / beta.shape[0]  # Weight for complexity cost term
            )
        ...
        if self.training:
            ...
            rnn_loss_eval = self.recurrent_impute.sample_elbo(
                inputs=torch.cat((hiddens_forward, hiddens_backward), dim=2),
            ...
            feat_loss_eval = self.feature_impute.sample_elbo(
            ...
            fusion_loss_eval = self.fuse_imputations.sample_elbo(
                inputs=torch.cat((gamma, masks), dim=-1),
                labels=normalized_values * data['eval_masks'],
                criterion=criterion_loss,
                sample_nbr=sample_nbr,  # Number of samples from the posterior distribution
                complexity_cost_weight=1 / beta.shape[0]  # Weight for complexity cost term
            )
